...
const loadRules = [
  ...
  body('drop').trim().notEmpty().withMessage('Drop location is required')
    .custom((drop, { req }) => {
      if (drop.trim().toLowerCase() === req.body.pickup?.trim().toLowerCase()) {
  ...
  body('preferredDate').isISO8601().withMessage('Valid date required')
    .custom((date) => {
      ...
      if (new Date(date) < today) throw new Error('Preferred date must be today or in the future');
      return true;
    }),
...
const availabilityRules = [
  ...
  body('availableDate').isISO8601().withMessage('Valid date required')
    .custom((date) => {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      if (new Date(date) < today) throw new Error('Available date must be today or in the future');
      return true;
    }),
  body('ratePerKm').isFloat({ min: 1 }).withMessage('Rate per km must be at least ₹1'),
];
